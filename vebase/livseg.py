    ...
    def main():
        ...
        def find_all_div_nodes(los):
            ...
            for i in range(0,len(los)):
                ...
                try:
                    if potroots[los[i][0]-1].left.vol > 150000 and (potroots[los[i][0]-1].left.vol > tempvol*0.41 and potroots[los[i][0]-1].left.vol < tempvol*0.76):
                        div_nodes.append(potroots[los[i][0]-1].val)
                except:
                    None
                try:
                    if potroots[los[i][0]-1].right.vol > 150000 and (potroots[los[i][0]-1].right.vol > tempvol*0.41 and potroots[los[i][0]-1].right.vol < tempvol*0.76):
                        div_nodes.append(potroots[los[i][0]-1].val)
                except:
                    None
            return div_nodes
        ...
        if tree_graph_check == 1:
            ...
            tree_pic = visualize_tree_vol(root)
            # Save as png file
            tree_pic.format = 'png'
        ...
        for i in range(0,len(res_div_nodes_n)):
            temp_arx = []
            final_array_segments.append(build_segments(potroots[res_div_nodes_n[i]-1],temp_arx,orgroot))
        final_array_segments.append(build_last(orgroot,temp_arx))
...
def vein_b_viz_l(porta,segs,stats,dist_map_final_liver_vol):
    ...
    for i in range(0,len(arr_objcts)):
        for cnt in range(0,len(arr_objcts[i])):
            ax.scatter3D(arr_objcts[i][cnt][2],arr_objcts[i][cnt][1],arr_objcts[i][cnt][0],color = color[i])
    plt.savefig('C:/Users/user/Desktop/diskuse', format='eps')
